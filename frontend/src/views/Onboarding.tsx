      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_17rem] lg:items-start">
              ...
              >
                <Card className="overflow-hidden">
                  <div className="p-5 sm:p-6">
                    ...
                    <h2 className="max-w-[32ch] text-wrap-pretty font-display text-xl leading-snug">
                      {current.question}
                    </h2>
                    {current.rationale && (
                      <p className="mt-2 max-w-[65ch] text-[12.5px] text-ink-soft italic">
                        Why this helps: {current.rationale}
                      </p>
                    )}
                  </div>
                  <form
                    className="border-t border-line-soft bg-paper/60 p-5 sm:p-6"
                    onSubmit={(event) => {
                  ...
                  >
                    ...
                    <textarea
                      ...
                      className="w-full resize-y rounded-lg border border-line bg-surface px-3 py-2.5 text-[14px] leading-relaxed outline-none placeholder:text-ink-soft focus-visible:border-moss focus-visible:ring-1 focus-visible:ring-moss"
                    />
                    <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <span id={helpId} className="max-w-[52ch] text-[12px] leading-relaxed text-ink-soft">
                        {current.optional
        ...
        <aside className="border-y border-line py-5 lg:sticky lg:top-8" aria-label="Question queue">
          <div className="flex items-baseline justify-between gap-3">
            <h2 className="font-display text-[15px] font-medium">Question queue</h2>
          ...
          {queuedQuestions.length > 0 ? (
            <ol className="mt-3">
              {queuedQuestions.map((question, index) => (
                <li key={`${question.section}.${question.key}`} className="flex gap-3 border-t border-line-soft py-3 first:border-t-0">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-line-soft text-[10.5px] font-semibold tabular-nums text-ink-soft">
                    {index + 2}
                  </span>
                  <div className="min-w-0">
          ...
          <div className="mt-4 border-t border-line-soft pt-4">
            ...
            {generate.isSuccess && generate.data.length === 0 && (
              <span role="status" className="mt-2 block text-[12px] text-ink-soft">
